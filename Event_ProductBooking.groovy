...
globalMap.put("ReplaceEventProductBookingPersonalData", new ReplaceEventProductBookingPersonalData())

/**
 * This class is used to replace or encrypt the personal data in a SQL resultset row
 */
class ReplaceEventProductBookingPersonalData{
	...
	static void replacePersonalData(row){
		println("Starting processing " + row.BOOKINGID )
		...
		if (row.CONTACTDETAILS) { row.CONTACTDETAILS = replaceContactPersonalData(row.CONTACTDETAILS, null) }
		println("Finished processing " + row.BOOKINGID )
	}




	static String replaceContactPersonalData(String xml, String xmldata){
		//GPathResult Contact = new XmlSlurper(false, false, false).parseText(xml)
		GPathResult Contact = new XmlSlurper().parseText(xml).declareNamespace(ota: 'http://www.opentravel.org/OTA/2003/05')
		boolean updated = false
		Contact.Travelers.Traveler.each {
		...
		if(updated){return XmlUtil.serialize(Contact)}
